...
function normalizeSide(side) {
  ...
  if (value === 'YES' || value === 'OVER') return 'OVER';
  if (value === 'NO' || value === 'UNDER') return 'UNDER';
...
function consensusKey(signal) {
  return [
    String(signal.player || '').trim().toLowerCase(),
    String(signal.market || '').trim().toLowerCase()
  ].join('|');
}

function consensusStrength(total, agree, disagree) {
  if (total <= 1) return 'SINGLE';
  if (agree === disagree) return 'SPLIT';
  if (total === 2) return 'EMERGING';
...
function buildConsensusProps(signals, sources) {
  ...
  return Array.from(groups.values()).map(group => {
    ...
    const overSignals = group.filter(
      signal => normalizeSide(signal.side) === 'OVER'
    );

    const underSignals = group.filter(
      signal => normalizeSide(signal.side) === 'UNDER'
    );
    ...
    if (over > under) {
      ...
      majoritySide = normalizeSide(first.side);
      agree = over;
      disagree = under;
    }
    ...
    const analysts = group.map(signal => {
    ...
    const majoritySignals = group.filter(
      signal =>
        normalizeSide(signal.side) === majoritySide
    );

    const majorityLines = majoritySignals
    ...
    const line = majorityLines.length
      ? majorityLines[0]
      : first.line;
    ...
    if (uniqueLines.length === 1) {
      lineDisplay = String(uniqueLines[0]);
    ...
    return {
      id:
        `CONS-${group.map(signal => signal.id).join('-')}`,
...
function configureConsensusFilter() {
  ...
  if (label) {
    label.textContent = 'Consensus Strength';
  }
}
...
async function load() {
  try {
    ...
    const sources = signalsData.sources || [];

    board = {
      mode: 'public-consensus',
      week: signalsData.week,
      season: signalsData.season,
      refreshedAt: new Date().toISOString(),
      sources,
      rawSignals: signals,

      props: buildConsensusProps(
        signals,
        sources
      )
    };
  ...
  return board.rawSignals
    .filter(signal => {
      ...
      return prop.analysts.some(
        analyst =>
          analyst.sourceId === signal.sourceId
      );
    })
...
function getLiveMatches(prop) {
  return getSignalIdsForProp(prop)
    .map(id => liveResults[id])
    .filter(Boolean);
}

function getResult(prop) {
  const matches = getLiveMatches(prop);
  ...
  const misses = matches.filter(
    result => result.status === 'MISS'
  ).length;
...
function liveStatus(prop) {
  const matches = getResultDetails(prop);
  ...
  return `
    <div class="live-stat pending">
      <strong>
        ${hits.length} HIT • ${misses.length} MISS
      </strong>
    </div>
  `;
}
